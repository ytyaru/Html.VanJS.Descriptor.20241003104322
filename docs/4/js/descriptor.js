...
class DyClass { // DyClass.make('ClassName', ['name', 'age'])
    static make(name, props) { // name:className, props:[descriptorName,...]
        ...
        if (Array.isArray(props) && props.every(v=>'string'===typeof v || v instanceof String)) {
            return this.#makeClass(name, props)
        }
        else { throw new TypeError(`propsは追加したいプロパティ名の配列であるべきです。:${typeof props} ${props}`) }
    }
    /*
    static #makeClass(name, props) {
    ...
    */
    ...
    static #makePropertyAssign(name,value='null'){return `${this.#makePropertyName(name)} = ${this.#makePropertyInitValue(value)};`}
    static #makePropertyName(name) { return `this._${name}` }
    static #makePropertyInitValue(value='null') { return `${value}` }

    static #makeDescriptors(names) { return names.map(n=>this.#makeDescriptor(n)).join('\n') }
